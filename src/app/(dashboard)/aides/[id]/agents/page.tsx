...
export default async function AideAgentsPage({
  params,
}: {
  ...
  const agents = await getAgentsByAideId(id);
      ...
      {leadAgent && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              The lead agent runs continuously and coordinates subordinate agents
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Link href={`/aides/${aide.id}/agents/${leadAgent.id}`}>
