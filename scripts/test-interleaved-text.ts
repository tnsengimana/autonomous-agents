...
const MODEL = process.env.LMSTUDIO_MODEL || "mistralai/ministral-3-14b-reasoning";
...
async function main() {
  ...
  // Simple test case
  const userMessage = "What's the current weather in San Francisco?";
  ...
  console.log("Top-level keys:", Object.keys(respAny).filter(k => !k.startsWith('_')));
  if ('reasoning' in respAny) {
    console.log(`Top-level reasoning: ${JSON.stringify(respAny.reasoning)?.substring(0, 300)}`);
  }
  if ('reasoningDetails' in respAny) {
    console.log(`Top-level reasoningDetails: ${JSON.stringify(respAny.reasoningDetails)?.substring(0, 300)}`);
  }
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      if ('content' in step && Array.isArray(step.content)) {
        ...
        for (const part of step.content as Array<Record<string, unknown>>) {
          ...
          if (part.type === 'reasoning') {
            console.log(`  Reasoning text: "${String(part.text || part.reasoning).substring(0, 150)}..."`);
      ...
      if ('response' in step && step.response) {
        ...
        if ('reasoning' in resp) {
          console.log(`Response.reasoning: ${JSON.stringify(resp.reasoning)?.substring(0, 300)}`);
        }
      }
      ...
      const toolResults = step.toolResults as Array<{ toolName: string }> | undefined;
      console.log(`toolResults: ${toolResults?.length || 0}`);
      if (toolResults) {
        for (const tr of toolResults) {
          const output = (tr as unknown as { output: unknown }).output;
          console.log(`  - ${tr.toolName}: ${JSON.stringify(output)?.substring(0, 100)}`);
        }
      }
    }
  }
  ...
  if (steps) {
    for (const step of steps) {
      ...
      if (stepAny.reasoning && typeof stepAny.reasoning === 'string' && stepAny.reasoning.trim()) {
        // Only add if not already added from content
        const alreadyAdded = events.some(e => 'llmThought' in e && e.llmThought === stepAny.reasoning);
        if (!alreadyAdded) {
  ...
  console.log(`- llmThought events: ${events.filter(e => 'llmThought' in e).length}`);
  console.log(`- llmOutput events: ${events.filter(e => 'llmOutput' in e).length}`);
  console.log(`- toolCalls events: ${events.filter(e => 'toolCalls' in e).length}`);
  console.log(`- toolResults events: ${events.filter(e => 'toolResults' in e).length}`);

  // Check if the model provides reasoning
  const hasReasoning = steps?.some(s => {
    const stepAny = s as Record<string, unknown>;
    return stepAny.reasoning && Array.isArray(stepAny.reasoning) && stepAny.reasoning.length > 0;
  });

  console.log(`\n--- Reasoning Support ---`);
  console.log(`Model provides reasoning: ${hasReasoning ? 'YES' : 'NO'}`);
  if (!hasReasoning) {
    console.log("This model's reasoning field is empty. All text goes to llmOutput.");
    console.log("For reasoning models (DeepSeek-R1, o1), reasoning would appear in llmThought.");
  }
